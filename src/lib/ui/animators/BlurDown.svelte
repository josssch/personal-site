...
<div
    use:animateController={{ animation, keyframes: 'blur-down' }}
    bind:this={controller}
    class="not-motion-reduce:-translate-y-4 not-motion-reduce:opacity-0 not-motion-reduce:blur-md"
>
    {@render children()}
</div>

<style>
    @keyframes -global-blur-down {
        to {
            filter: blur(0);
            opacity: 1;
            translate: 0 0;
